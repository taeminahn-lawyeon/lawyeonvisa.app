...
- 판결 뉴스레터는 `주요 쟁점 → 시사점`, 제도·매뉴얼 변경 뉴스레터는 `주요 변경 → 시사점`으로 구성한다. 실제 개정을 확인하지 못한 현행 기준 해설은 `주요 내용 → 시사점`으로 쓴다. 그 아래 항목은 구체적인 쟁점·변경 사항의 명사구로 쓴다. 글마다 필요한 항목 수와 설명 분량은 다르게 두고, 짧은 업무사례에 소제목을 억지로 만들지 않는다.
...
- 판결의 `주요 쟁점`에서는 다툼, 법원의 결론, 결론을 만든 사실과 법리를 설명한다. `시사점`에서는 그 결론에 따라 유사한 상황의 독자가 취할 수 있는 신청·주장·증명·불복 또는 업무상 선택을 도출한다. 판결 이유를 다시 요약하지 않는다.
- 시사점의 적용 대상과 가능한 대응을 구체화한다. 이혼 확정 전 F-6-3 요건을 인정한 판결이라면, 이혼소송 중 체류기간이 만료되는 사람이 파탄의 주된 책임을 근거로 체류 연장을 구할 수 있다는 적용까지 설명한다. 이 예시를 다른 글의 고정 양식으로 사용하지 않는다.
- 제도 변경의 `주요 변경`은 전후 기준, 대상, 시행 시점과 경과조치를 확인한 범위에서 설명한다. `시사점`은 달라진 기준 때문에 신청 가능성·시점·증빙·운영상 선택이 어떻게 달라지는지 설명한다. 기존 기준을 처음 발견했다는 이유로 새 제도라고 소개하지 않는다.
- 법원의 판단과 필자의 적용을 구별하고, 그 적용을 실제로 제한하는 조건·기간·사실관계를 대응과 함께 적는다. 하급심 판단을 일반적으로 확정된 법리로 확대하지 않는다. ‘자동으로 허가되는 것은 아니다’라는 단서를 모든 글에 반복하지 않는다.
- 시사점만 읽고도 누가 무엇을 주장하거나 선택할 수 있는지 알 수 있어야 한다. ‘의미 있는 판결’, ‘유사 사건에 참고’, ‘자료를 충분히 준비’만으로 마무리하지 않는다. 글마다 필요한 분석만 전개하고 증명방법·신청 시점·구제수단을 모두 넣으려 하지 않는다.
- 김앤장 뉴스레터의 개별 글에서 도출한 설명 방식과 이번 반영 범위는 [뉴스레터 시사점 연구 메모](newsletter-implications-research-20260918.md)에 기록한다. 이를 통일된 문단 틀로 사용하지 않는다.
