...
`send-admin-email` 함수에 **사전상담 분기**가 새로 들어갔습니다.
이걸 배포하지 않으면 접수는 저장되지만 **메일이 오지 않습니다.**
(정확히는 함수가 500 오류를 냅니다 — 옛 코드에는 사전상담을 처리하는 분기가 없기 때문입니다.)
...
| 본문 | `· Email: ...` 에 신청자 주소가 있고, 누르면 새 메일 작성창이 열린다 |
| 대시보드 | `admin-dashboard.html` → **사전상담** 탭에 해당 건이 보이고 뱃지에 숫자가 뜬다 |

회신은 본문의 신청자 주소로 **직접 새 메일을 보내는 방식**입니다.
(알림 메일에 그대로 답장하면 발신 주소인 `noreply@lawyeonvisa.app` 로 갑니다.)
...
| 함수 호출이 `500` | 2단계 배포 미실행이 가장 흔한 원인. 옛 함수에는 사전상담 분기가 없어 오류를 냅니다. Functions → Logs 에서 실제 메시지 확인 |
